...
public class JavaTask3 extends HttpServlet {
	...
	void case_2(HttpServletRequest req, HttpServletResponse resp)
		...
		try {
			...
			if (Math.abs(int_m - 10) < Math.abs(int_n - 10)) {
			...
			resp.getWriter().println("Вводите числовые значения в поля");
		}
	}
	...
	void case_5(HttpServletRequest req, HttpServletResponse resp)
		...
		try {
			...
			if (d>0){
				...
				x1=-(int_b+d)/(2*int_a);
				x2=-(int_b-d)/(2*int_a);
				resp.getWriter().println("<p>Первый корень X="+x1+"</p><p>Второй корень X="+x2+"</p>");
			}
			else if(d==0){
				x1=-(int_b)/(2*int_a);
				resp.getWriter().println("<p>D=0, уравнение имеет только 1 корень</p>"+"<p>X="+x1+"</p>");
			}
			else if(d<0){
				resp.getWriter().println("<p>D<0, корней нет</p>");
			}
		} catch (NumberFormatException n) {
